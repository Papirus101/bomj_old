...
from image_generate.works.dvor import generate_image
...
async def send_work(call):
    ...
    await call.message.answer_photo(
        open(f'{pathlib.Path().absolute()}/image/works/dvor/{call.from_user.id}_dvor.png', 'rb'),
        reply_markup=await work_check('2', 'Уволиться'))
    pathlib.Path(f'{pathlib.Path().absolute()}/image/works/dvor/{call.from_user.id}_dvor.png').unlink()
...
async def work_info(call: types.CallbackQuery, callback_data: dict):
    ...
    await call.message.answer_photo(open(f'{pathlib.Path().absolute()}/image/works/dvor/dvor.png', 'rb'),
                                    caption="🧹 Тебя отправили в местный двор для работы дворником.\n"
                                            f"Тебе платят за каждую убранную мусорку {convert_stats(money=user.lvl * 15 if user.lvl > 0 else 15)} руб.\n\n"
...
async def start_work(call: types.CallbackQuery):
    ...
    if not await check_user_characteristics(db_session, call):
        await call.message.answer('Ты не можешь работать, проверь свои показатели')
        return
...
async def check_user_answer(call: types.CallbackQuery, callback_data: dict):
    ...
    if user.event_id == int(user_answer):
        ...
        event_text = f'💰 Ты убрал необходимую мусорку, твой заработок {convert_stats(money=total_reward)} руб.'
    else:
        event_text = 'Ты убрал не ту мусорку, работа продолжается...'
    time_to_sleep = random.randint(3, 6)
    system_message = await call.message.answer(f'{event_text}\n'
                                               f'{random_events_text}\n'
                                               f'🏃 <code>Ты пошёл к следующей мусорке, дорога займёт {time_to_sleep} с.</code>')
...
def register_work_dvor_handler(dp: Dispatcher):
    dp.register_callback_query_handler(work_info, works_callback.filter(event='info', type='2'), chat_type='private')
    dp.register_callback_query_handler(start_work, works_callback.filter(event='start', type='2'), chat_type='private')
    dp.register_callback_query_handler(check_user_answer, works_callback.filter(type='2_check'), chat_type='private')